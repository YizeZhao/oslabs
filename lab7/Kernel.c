...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
      ...
      newTh.stackTop = & (newTh.systemStack[SYSTEM_STACK_SIZE-1])
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
      ...
      -- find processid's child process that is not free
         ...
         && (processManager.processTable[i].status != FREE)
         && (currentThread.myProcess.pid == processManager.processTable[i].parentsPid))
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
      ...
      -- AllocateanOpenFileobject(seeFileManager.Open)
      newOF = fileManager.Open(&strBuffer)
      if newOF == null
        return -1
      endIf

      -- Set the entry in the fileDescriptor array to point to the OpenFile object
      currentThread.myProcess.fileDescriptor[idx] = newOF
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      ...
      readingOF = currentThread.myProcess.fileDescriptor[fileDesc]
      ...
      while true
        ...
        readRet = fileManager.SynchRead(readingOF, destAddr, nextPosInFile, thisChunkSize)
      ...
      readingOF.currentPos = nextPosInFile
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      ...
      while true
        ...
        currentThread.myProcess.addrSpace.SetReferenced(virtPage)
        destAddr = currentThread.myProcess.addrSpace.ExtractFrameAddr(virtPage) + offset
        writeRet = fileManager.SynchWrite(readingOF, destAddr, nextPosInFile, thisChunkSize)
...
-----------------------------  StartUserProcess  ---------------------------------
  ...
  function StartUserProcess ()
    ...
    pOF = fileManager.Open("TestProgram4")
